...
export const moviesApiSlice = createApi({
  ...
  baseQuery: fetchBaseQuery({ baseUrl: `https://api.themoviedb.org/3/` }) as BaseQueryFn<string | FetchArgs, unknown, MoviesApiError, {}>,
  endpoints: (builder) => ({

    getMovies: builder.query<MoviesSearchApiResponse, SearchData>({
      query: ({ searchTerm, page = 1 }) => `search/movie?api_key=0c3f0750ff16ca7c4b32ebb707051867&append_to_response=credits&query=${searchTerm}&page=${page}`,
      ...
      transformResponse: (response: MoviesSearchApiResponse) => {
    ...
    getMovieDetails: builder.query<MovieDetails, string | undefined>({
      query: (id) => {
        ...
        return `movie/${id}?api_key=0c3f0750ff16ca7c4b32ebb707051867&append_to_response=credits,videos`
      },
      transformResponse: (response: MovieDetails) => {
        ...
        response.credits.crew = response.credits.crew.filter(person => {
          return ["Writer", "Director", "Producer", "Characters", "Screenplay"].includes(person.job)
        })

        // store genre ids, movies api query returns different results for genres in different queries
        response.genre_ids = response.genres.map(genre => genre.id)

        // show only actors with profile image
        response.credits.cast = response.credits.cast.filter(person => person.profile_path)
        return response
      }
    }),
    ...
    nowPlaying: builder.query<MovieDetails[], void>({
      query: () => `movie/now_playing?api_key=0c3f0750ff16ca7c4b32ebb707051867&page=1`,
      transformResponse: (response: MoviesSearchApiResponse) => {
    ...
    topRated: builder.query<MovieDetails[], void>({
      query: () => `movie/top_rated?api_key=0c3f0750ff16ca7c4b32ebb707051867&page=1`,
      transformResponse: (response: MoviesSearchApiResponse) => {
    ...
    similarMovies: builder.query<MovieDetails[], number>({
      query: (id) => `movie/${id}/similar?api_key=0c3f0750ff16ca7c4b32ebb707051867&page=1`,
      transformResponse: (response: MoviesSearchApiResponse) => {
        return response.results.slice(0, 6)
      }
    }),
...
export const useMovieQueryError = (isError: boolean, error: MoviesApiError | SerializedError | undefined) => {
  if (isError && error) {
    if ('data' in error) {
...
export const { useLazyGetMoviesQuery, useGetMovieDetailsQuery, useGetActorDetailsQuery, useGetActorCreditsQuery, useNowPlayingQuery, useTopRatedQuery, useSimilarMoviesQuery, useGetGenresQuery, } = moviesApiSlice
