...
describe("Grid card", () => {
  ...
  it("should show movie image if present", () => {
    ...
    expect(screen.getByRole("img", { name: movieWithImage.title })).toBeInTheDocument()

    act(() => {
      renderResult.rerender(<GridCard movie={movieWithoutImage} />)
    })
    expect(screen.queryByRole("img", { name: movieWithoutImage.title })).not.toBeInTheDocument()
    expect(screen.getByRole("heading", { name: movieWithoutImage.title })).toBeInTheDocument()
  })
  ...
  it("should link to movie detail", () => {
    ...
    expect(screen.getByRole('link', { name: /movie detail link/i }).getAttribute('href')).toEqual(`/movie/${movieWithImage.id}`)
  })
  ...
  it("should have hidden popup on load and short hover", () => {
    ...
    expect(renderResult.container.querySelector('.card-popup')).not.toBeVisible()
