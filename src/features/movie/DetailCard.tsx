 ...
 */
const DetailCard: React.FC<{ movie: MovieDetails }> = React.memo(({ movie }) => {
  const theme = useTheme();
  const poster = movie.poster_path ? `https://image.tmdb.org/t/p/w200/${movie.poster_path}` : "";
  const backdrop = movie.backdrop_path ? `https://image.tmdb.org/t/p/w500/${movie.backdrop_path}` : poster;
  return (
    <MovieContext.Provider value={movie}>
      <Box className="movie-item detail-type"
        sx={{
          position: "relative",
          borderRadius: 1,
          overflow: "hidden",
        ...
        <Link href={`/movie/${movie.id}`} underline="none">
          ...
          {poster && <ImageWithOverlay image={backdrop} title={movie.title} color={theme.palette.background.defaultAlt} />}

          <FavouriteButton size="large" floating />

          <Box sx={{ p: theme.spacing(2) }} >
            {poster
              ? <Stack spacing={2} position="relative" flexWrap="nowrap" alignItems="flex-end" sx={{
                mb: theme.spacing(2),
                mt: "-150px"
              }}>
                <Box
                  ...
                  sx={{ borderRadius: 1 }}
                />
                <Typography variant="h3" sx={{ mb: 0 }}>{movie.title}</Typography>
              </Stack>
              : <Typography variant="h3" sx={{ mb: theme.spacing(2) }}>{movie.title}</Typography>
            }
