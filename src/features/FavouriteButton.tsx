 ...
 */
const FavouriteButton: React.FC<FavouriteButtonType> = ({ floating = false, ...props }) => {
  const theme = useTheme();
  const dispatch = useDispatch();
  const movie = useContext(MovieContext)
  const isFavourite = useIsFavouriteMovie(movie.id)

  const handleFavourite = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
  ...
  return (
    <CustomTooltip title={isFavourite ? "Remove from favourites" : "Add to favourites"} placement="left" >
      <IconButton aria-label={isFavourite ? "remove movie from favourites" : "Add to favourites"} onClick={handleFavourite}
        sx={{
          ...(floating && {
            position: 'absolute',
            top: 5,
        ...
        {...props}
      >
        <FavoriteIcon color="secondary" sx={{ ...(isFavourite && { color: "primary.fancy" }) }} />
      </IconButton>
    </CustomTooltip>
  )
