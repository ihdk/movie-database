...
describe("Search results", () => {
  it("should render with header text when movies found", () => {
    ...
    act(() => {
      renderWithProvider(<SearchResults />, {
        preloadedState: {
          local: { ...initialState, movies: data, totalMovies: total },
        },
      });
    });
  ...
  it("should render movies results in appropriate view", () => {
    ...
    let renderResult;
    act(() => {
      renderResult = renderWithProvider(<SearchResults />, {
        preloadedState: {
          local: { ...initialState, movies: data, searchResultsView: "grid" },
        },
      });
    });
    for (let movie of data) {
      if (movie.poster_path) {
        expect(
          screen.getByRole("img", { name: movie.title })
        ).toBeInTheDocument();
      } else {
        expect(
    ...
    act(() => {
      renderResult.rerender(<SearchResults />, {
        preloadedState: {
          local: { ...initialState, movies: data, searchResultsView: "list" },
        },
      });
    });
    for (let movie of data) {
      expect(
        screen.queryByRole("img", { name: movie.title })
  ...
  it("should be visible load more button", () => {
    ...
    expect(
      screen.getByRole("button", { name: /15 more movies/i })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: /5 more movies/i })
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("button", { name: /more movies/i })
    ).not.toBeInTheDocument();
  });
});
