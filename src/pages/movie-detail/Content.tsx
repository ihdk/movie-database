          ...
          <Grid item md={8} sm={7} xs={12}>
            ...
            {movie.overview &&
              <Section spacing="tiny" component="div" disableGutters>
                ...
                <Typography sx={{ whiteSpace: "pre-line" }}>{movie.overview}</Typography>
              </Section>
            }

            {movie.vote_average > 0 &&
              <Section spacing="tiny" component="div" disableGutters>
                <MovieScore value={movie.vote_average * 10} votes={movie.vote_count} isMovieContent />
              </Section>
            }
  ...
  return (
    <>
      <Typography variant="h1" sx={{
        mt: movie.poster_path ? theme.spacing(6) : 0,
        mb: 0
      }}
      >{movie.title}</Typography>

      {movie.tagline && <Typography variant="subtitle2" component="h2" color={theme.palette.primary.fancy}>{movie.tagline}</Typography>}
    </>
  )
})
...
const DataLine: React.FC = React.memo(() => {
  ...
  return (
    ...
    >
      ...
      {movie.genre_ids && <MovieGenres genresIds={movie.genre_ids} isMovieContent />}

      {movie.runtime && <Typography>{movie.runtime} min</Typography>}
      {movie.release_date && <Typography>{dayjs(movie.release_date).format('DD. MMM YYYY')}</Typography>}
    </Stack >
  )
})
    ...
    <Stack >
      {Object.keys(positions).map((name) => {
        return <Box key={`${name}`} sx={{
          ...
          <Typography variant="body2" color="text.secondary">{positions[name].join(', ')}</Typography>
        </Box>
      })}
    </Stack>
  )
...
const Actors: React.FC = React.memo(() => {
  ...
  return movie.credits.cast.length > 0
    ? <Section fullwidth>
      ...
      <Swiper slides={movie.credits.cast.map(person => <ActorCard key={person.id} actor={person} />)} />
    </Section>
    : null
})
