...
const DataLine: React.FC = React.memo(() => {
  ...
  return (
    <Stack spacing={2} divider={<Divider orientation="vertical" flexItem />}>
      ...
      {actor.birthday && (
        <Typography>
          Born {dayjs(actor.birthday).format("DD. MMM YYYY")}
        </Typography>
      )}
      {actor.deathday && (
        <Typography>
          Died in {dayjs(actor.deathday).format("DD. MMM YYYY")}
        </Typography>
      )}
    </Stack>
  );
...
export const ActorPopularity: React.FC<{ value: number }> = React.memo(
  ({ value }) => {
    return (
      <Stack spacing={1}>
